...
var Pair = /*#__PURE__*/function () {
  ...
  _proto.getOutputAmount = function getOutputAmount(inputAmount) {
    ...
    var outputAmount = CurrencyAmount$1.fromRawAmount(inputAmount.currency.equals(this.token0) ? this.token1 : this.token0, JSBI.divide(numerator, denominator));
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount) {
    ...
    var inputAmount = CurrencyAmount$1.fromRawAmount(outputAmount.currency.equals(this.token0) ? this.token1 : this.token0, JSBI.add(JSBI.divide(numerator, denominator), ONE));
  ...
  _proto.getLiquidityValue = function getLiquidityValue(token, totalSupply, liquidity, feeOn, kLast) {
    ...
    return CurrencyAmount$1.fromRawAmount(token, JSBI.divide(JSBI.multiply(liquidity.quotient, this.reserveOf(token).quotient), totalSupplyAdjusted.quotient));
  };
    ...
    get: function get() {
      ...
      for (var _iterator2 = _createForOfIteratorHelperLoose(this.pairs.entries()), _step2; !(_step2 = _iterator2()).done;) {
        var _step2$value = _step2.value,
            i = _step2$value[0],
            pair = _step2$value[1];
        prices.push(this.path[i].equals(pair.token0) ? new Price$1(pair.reserve0.currency, pair.reserve1.currency, pair.reserve0.quotient, pair.reserve1.quotient) : new Price$1(pair.reserve1.currency, pair.reserve0.currency, pair.reserve1.quotient, pair.reserve0.quotient));
      }
  ...
  Fetcher.fetchPairData = function fetchPairData(tokenA, tokenB, provider) {
    try {
      ...
      return Promise.resolve(new Contract(address, IUniswapV2Pair.abi, provider).getReserves()).then(function (_ref) {
        ...
        return new Pair(CurrencyAmount$2.fromRawAmount(tokenA, balances[0]), CurrencyAmount$2.fromRawAmount(tokenB, balances[1]));
      });
    } catch (e) {
      return Promise.reject(e);
    }
