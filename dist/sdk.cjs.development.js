...
var Pair = /*#__PURE__*/function () {
  function Pair(currencyAmountA, tokenAmountB) {
    ...
    this.liquidityToken = new token.Token(tokenAmounts[0].currency.chainId, Pair.getAddress(tokenAmounts[0].currency, tokenAmounts[1].currency), 18, 'UNI-V2', 'Uniswap V2');
  ...
  _proto.getOutputAmount = function getOutputAmount(inputAmount) {
    ...
    var outputAmount = fractions.CurrencyAmount.fromRawAmount(inputAmount.currency.equals(this.token0) ? this.token1 : this.token0, JSBI.divide(numerator, denominator));
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount) {
    ...
    var inputAmount = fractions.CurrencyAmount.fromRawAmount(outputAmount.currency.equals(this.token0) ? this.token1 : this.token0, JSBI.add(JSBI.divide(numerator, denominator), ONE));
  ...
  _proto.getLiquidityMinted = function getLiquidityMinted(totalSupply, tokenAmountA, tokenAmountB) {
    ...
    if (JSBI.equal(totalSupply.quotient, ZERO)) {
      liquidity = JSBI.subtract(utils.sqrt(JSBI.multiply(tokenAmounts[0].quotient, tokenAmounts[1].quotient)), MINIMUM_LIQUIDITY);
    } else {
    ...
    return fractions.CurrencyAmount.fromRawAmount(this.liquidityToken, liquidity);
  };
  ...
  _proto.getLiquidityValue = function getLiquidityValue(token, totalSupply, liquidity, feeOn, kLast) {
    ...
    if (!feeOn) {
      ...
      if (!JSBI.equal(kLastParsed, ZERO)) {
        var rootK = utils.sqrt(JSBI.multiply(this.reserve0.quotient, this.reserve1.quotient));
        var rootKLast = utils.sqrt(kLastParsed);

        if (JSBI.greaterThan(rootK, rootKLast)) {
          var numerator = JSBI.multiply(totalSupply.quotient, JSBI.subtract(rootK, rootKLast));
          var denominator = JSBI.add(JSBI.multiply(rootK, FIVE), rootKLast);
          var feeLiquidity = JSBI.divide(numerator, denominator);
          totalSupplyAdjusted = totalSupply.add(fractions.CurrencyAmount.fromRawAmount(this.liquidityToken, feeLiquidity));
        } else {
          totalSupplyAdjusted = totalSupply;
        }
      } else {
    ...
    return fractions.CurrencyAmount.fromRawAmount(token, JSBI.divide(JSBI.multiply(liquidity.quotient, this.reserveOf(token).quotient), totalSupplyAdjusted.quotient));
  };
  ...
  _createClass(Pair, [{
    ...
    get: function get() {
      ...
      return new fractions.Price(this.token0, this.token1, result.denominator, result.numerator);
    }
    /**
    ...
    get: function get() {
      ...
      return new fractions.Price(this.token1, this.token0, result.denominator, result.numerator);
    }
  }, {
    key: "chainId",
    get: function get() {
    ...
    get: function get() {
      ...
      for (var _iterator2 = _createForOfIteratorHelperLoose(this.pairs.entries()), _step2; !(_step2 = _iterator2()).done;) {
        var _step2$value = _step2.value,
            i = _step2$value[0],
            pair = _step2$value[1];
        prices.push(this.path[i].equals(pair.token0) ? new fractions.Price(pair.reserve0.currency, pair.reserve1.currency, pair.reserve0.quotient, pair.reserve1.quotient) : new fractions.Price(pair.reserve1.currency, pair.reserve0.currency, pair.reserve1.quotient, pair.reserve0.quotient));
      }

      var reduced = prices.slice(1).reduce(function (accumulator, currentValue) {
        return accumulator.multiply(currentValue);
      }, prices[0]);
      return this._midPrice = new fractions.Price(this.input, this.output, reduced.denominator, reduced.numerator);
    }
  }, {
    key: "chainId",
    get: function get() {
...
var Trade = /*#__PURE__*/function () {
  function Trade(route, amount, tradeType) {
    ...
    if (tradeType === exports.TradeType.EXACT_INPUT) {
      ...
      this.inputAmount = fractions.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
      this.outputAmount = fractions.CurrencyAmount.fromFractionalAmount(route.output, tokenAmounts[tokenAmounts.length - 1].numerator, tokenAmounts[tokenAmounts.length - 1].denominator);
    } else {
      ...
      this.inputAmount = fractions.CurrencyAmount.fromFractionalAmount(route.input, tokenAmounts[0].numerator, tokenAmounts[0].denominator);
      this.outputAmount = fractions.CurrencyAmount.fromFractionalAmount(route.output, amount.numerator, amount.denominator);
    }
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance) {
    ...
    if (this.tradeType === exports.TradeType.EXACT_OUTPUT) {
      ...
      var slippageAdjustedAmountOut = new fractions.Fraction(ONE).add(slippageTolerance).invert().multiply(this.outputAmount.quotient).quotient;
      return fractions.CurrencyAmount.fromRawAmount(this.outputAmount.currency, slippageAdjustedAmountOut);
    }
  }
  /**
  ...
  _proto.maximumAmountIn = function maximumAmountIn(slippageTolerance) {
    ...
    if (this.tradeType === exports.TradeType.EXACT_INPUT) {
      ...
      var slippageAdjustedAmountIn = new fractions.Fraction(ONE).add(slippageTolerance).multiply(this.inputAmount.quotient).quotient;
      return fractions.CurrencyAmount.fromRawAmount(this.inputAmount.currency, slippageAdjustedAmountIn);
    }
  }
  /**
  ...
  _proto.worstExecutionPrice = function worstExecutionPrice(slippageTolerance) {
    return new fractions.Price(this.inputAmount.currency, this.outputAmount.currency, this.maximumAmountIn(slippageTolerance).quotient, this.minimumAmountOut(slippageTolerance).quotient);
  }
  /**
  ...
  Fetcher.fetchPairData = function fetchPairData(tokenA, tokenB, provider) {
    try {
      ...
      return Promise.resolve(new contracts.Contract(address, IUniswapV2Pair.abi, provider).getReserves()).then(function (_ref) {
        ...
        return new Pair(entities.CurrencyAmount.fromRawAmount(tokenA, balances[0]), entities.CurrencyAmount.fromRawAmount(tokenB, balances[1]));
      });
    } catch (e) {
      return Promise.reject(e);
    }
